  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      for (const docSnap of snapshot.docs) {
        ...
        const isUnread = lastSeen ? new Date(lastUpdated) > new Date(lastSeen) : !!lastUpdated;
...
const styles = StyleSheet.create({
  ...
  pageTitle: { fontSize: 22, fontWeight: 'bold', marginBottom: 15, color: '#00796B' },
  searchInput: {
    flex: 1,
  ...
  emptyText: { color: '#999', fontSize: 16, textAlign: 'center', paddingHorizontal: 20 },
  discoverLink: {
    marginTop: 12,
    color: '#FF7043',
    fontSize: 16,
