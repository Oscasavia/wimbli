...
export default function Chat() {
  ...
  const inputBackgroundColor = currentTheme.inputBackground || (theme === 'dark' ? '#2c2c2e' : '#f0f0f0');
  const inputBorderColor = currentTheme.inputBorder || (theme === 'dark' ? '#444' : '#ddd');
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
        console.log(`Chat snapshot received. Size: ${snapshot.size}, Changes: ${snapshot.docChanges().length}`);
        ...
        const msgsPromises = snapshot.docs.map(async (docSnap): Promise<Message> => {
  ...
  const sendMessage = async () => {
    ...
    const newMessageData: any = { // Use 'any' for flexibility or create a specific SendMessage type
      text: trimmedInput,
      senderId: userId,
    ...
    try {
        ...
        await addDoc(collection(db, "groups", groupId, "messages"), newMessageData);
  ...
  const renderMessageItem = ({ item }: { item: Message }) => {
    ...
    return (
      <Pressable // Use Pressable for better control over feedback/gestures
        ...
        delayLongPress={300}
        style={[styles.messageRow, { justifyContent: isSender ? 'flex-end' : 'flex-start' }]}
      >
         {/* Avatar for Receiver */}
         {!isSender && (
             <Image
                source={item.senderAvatar ? { uri: item.senderAvatar } : require("../assets/default-profile.png")}
                style={styles.avatar}
            />
        ...
        <View style={[styles.messageContentContainer, isSender ? styles.senderMessageAlign : styles.receiverMessageAlign]}>
            ...
            <View style={[
                ...
                {item.replyToMessageText && (
                    <View style={[styles.replyContainer, { borderLeftColor: isSender ? textSenderColor+'50' : replyBorderColor }]}>
                        <Text style={[styles.replySender, { color: isSender ? textSenderColor+'b0' : currentTheme.textSecondary }]}>
                            {item.replyToSenderName || 'Unknown'}
                        </Text>
                        <Text style={[styles.replyText, { color: isSender ? textSenderColor+'d0' : textReceiverColor+'d0' }]} numberOfLines={2}>
                            {item.replyToMessageText}
                        </Text>
                    </View>
                )}
                {/* Main Message Text */}
                <Text style={[styles.messageText, { color: isSender ? textSenderColor : textReceiverColor }]}>
                    {item.text}
                </Text>
                 {/* Timestamp & Likes Row */}
                 <View style={styles.bubbleFooter}>
                     ...
                     {(item.likedBy && item.likedBy.length > 0) && (
                        <Text style={[styles.likeText, { color: isSender ? timestampSenderColor : timestampReceiverColor }]}>
                           ❤️ {item.likedBy.length}
                        </Text>
                     )}
                     {/* Timestamp */}
                     <Text style={[styles.timestamp, { color: isSender ? timestampSenderColor : timestampReceiverColor }]}>
  ...
  return (
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: currentTheme.background }]} edges={['bottom']}>
      <KeyboardAvoidingView
      ...
      >
        ...
        {loadingMessages ? (
            <View style={styles.loadingContainer}>
             ...
             <View style={styles.loadingContainer}>
                <Text style={{ color: currentTheme.textSecondary }}>No messages yet. Start the conversation!</Text>
             </View>
        ) : (
            <FlatList
                ref={flatListRef}
        ...
        {/* Input Area */}
        <View style={[styles.inputAreaContainer, { borderTopColor: inputBorderColor }]}>
          <View style={[styles.inputWrapper, { backgroundColor: inputBackgroundColor, borderColor: inputBorderColor }]}>
            <TextInput
              value={input}
          ...
          <TouchableOpacity
            style={[styles.sendButton, { backgroundColor: input.trim() ? currentTheme.primary : currentTheme.textSecondary }]} // Dim button if no input
            onPress={sendMessage}
            disabled={!input.trim()} // Disable if no input
          >
            <Feather name="send" size={20} color={currentTheme.buttonText || '#fff'} />
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
    </SafeAreaView>
