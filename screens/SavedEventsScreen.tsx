        ...
        useCallback(() => {
            ...
            const fetchSavedEvents = async () => {
                ...
                if (!currentUser) {
                    ...
                    if (isActive) { setSavedEvents([]); setLoading(false); }
                    return;
                }
                ...
                try {
                    ...
                    if (!userDocSnap.exists()) {
                        ...
                        setSavedEvents([]); setLoading(false);
                        return;
                    }
                    ...
                    const eventPromises = savedPostIds.map(postId => getDoc(doc(db, "posts", postId)));
                    ...
                    const eventsData: Post[] = eventDocsSnaps
                        ...
                        .map(docSnap => ({
                            id: docSnap.id,
                            ...docSnap.data(),
                        } as Post)); // Basic mapping, assuming data matches Post type
                     ...
                     onPress: async () => {
                         ...
                         try {
                              ...
                              // Optimistically remove from local state for immediate UI update
                               setSavedEvents(prev => prev.filter(event => event.id !== postId));
        ...
        // Format date (copy or import the formatDate helper)
         ...
         try {
            ...
            if (!isNaN(dateObj.getTime())) {
                displayDate = dateObj.toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' }); // Full date might be better here
            }
                ...
                <View style={[styles.card, { backgroundColor: cardBackgroundColor }]}>
                    ...
                    <View style={styles.cardHeader}>
                        <Text style={[styles.cardTitle, { color: currentTheme.textPrimary }]} numberOfLines={2}>
                         ...
                         <TouchableOpacity onPress={() => handleUnsave(item.id)} style={styles.unsaveButton}>
                             <Feather name="bookmark" size={22} color={savedIconColor} fill={savedIconColor} />
                         </TouchableOpacity>
                    </View>
            ...
            <View style={[styles.screen, { backgroundColor: currentTheme.background }]}>
               ...
               <View style={[styles.screenB, { backgroundColor: currentTheme.background}]}>
                ...
                <Text style={[styles.emptyText, { color: currentTheme.textSecondary }]}>
                    You haven't saved any events yet. Find events on the Home screen and tap the bookmark icon!
                </Text>
...
const styles = StyleSheet.create({
  ...
  cardDescShort: { // Optional description style
    fontSize: 14,
    lineHeight: 20,
    marginTop: 8,
